...
#
# Dispatch the single vgi-grammar image into one of its transports:
...
#                    $VGI_UNIX_SOCK or /tmp/vgi-grammar.sock).
...
#
# The worker is stateless (LanguageTool + English rules baked into the JAR), so
...
case "${1:-http}" in
  ...
  unix)
    shift 2>/dev/null || true
    SOCK="${1:-${VGI_UNIX_SOCK:-/tmp/vgi-grammar.sock}}"
