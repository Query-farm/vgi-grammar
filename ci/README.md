# CI: the vgi-grammar worker integration suite
...
(`test/sql/*.test`) against the vgi-grammar VGI worker through the **real DuckDB
...
1. **Build the worker** — `./gradlew shadowJar` produces a self-contained fat
   JAR at `build/libs/vgi-grammar-<ver>-all.jar`. Its manifest sets `Add-Opens`, so
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   ...
   chosen transport, points `VGI_GRAMMAR_WORKER` at it, **warms the extension cache
...
`VGI_GRAMMAR_WORKER` resolves to. `run-integration.sh` is parameterized by a
...
| `TRANSPORT`  | `VGI_GRAMMAR_WORKER`       | how the worker is started | readiness gate |
|--------------|-------------------------|---------------------------|----------------|
...
VGI_GRAMMAR_WORKER="java -jar $JAR" \
...
VGI_GRAMMAR_WORKER_JAR="$JAR" ci/run-integration.sh
