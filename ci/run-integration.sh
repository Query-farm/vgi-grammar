...
#   VGI_GRAMMAR_WORKER    worker LOCATION the .test files attach (a stdio command
#                      such as `java -jar /abs/path/vgi-grammar-<ver>-all.jar`, or
#                      an http:// URL)
# Optional:
...
: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_GRAMMAR_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
echo "Running suite (worker: $VGI_GRAMMAR_WORKER) ..."
